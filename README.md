...
**The intake (handoffs of 8 Sep 2026: first-time users C/B/D, returning customers R, the two-minute set-up O).** The mechanism is record → transcribe → extract facts → chips → confirm; nobody proof-reads a transcript and a correction is a tap. `/welcome` (two doors) · `/say` (just say it; `?for=trip` is Trips' New trip with the profile's chips in grey, `?for=inspire` the "Or just ask" row; `?type=1` the keyboard) · `/say/steps` (one question at a time) · `/say/<id>` (the fact card; lime = said, grey = profile, dashed = gap; tap a chip for the picker) · `/say/<id>/ask` (at most two gap questions) · `/inspire?intake=<id>` (the chip row over the results, and the "Remember that…?" harvest card) · `/setup?step=1..5` (home, how far, who, food, what you like doing) · `/household/<id>/tell` and `/review` (one person by voice). Server: `domain/voiceFacts.js` (the fact set and the profile's defaults), `domain/voiceHousehold.js` (the household breaths), `voice_intakes` (migration 075 — facts and taps, never a transcript), and `POST /api/voice/intake`, `/household/who|food|likes|apply` in `routes/voice.js`. A trip made from the card goes through the ordinary `POST /api/trips`.
