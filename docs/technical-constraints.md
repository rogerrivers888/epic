...
**Two numbers, because they are two different things.** Ten hours is how long a session can be *read* — `livePlanSession` will not return a row past `expires_at`, so the content is unreachable from that moment. How long the row is *physically present* is longer: an hour's grace, so a household returning a minute late is told the session expired rather than having a row vanish mid-request, plus however long until the next sweep. That sweep was daily, which made the worst case about **thirty-five hours** depending on the time of day a session happened to be created; it is **hourly** now, so the honest figures are ten hours readable and at most twelve hours present. A paragraph meant to govern a licensing decision has to say both, and an interval is part of a retention policy whether or not anybody wrote it down as one.
