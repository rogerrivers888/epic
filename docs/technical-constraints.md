...
**Lanes A and B — the two private paths to a one-off** (owner, 13 Sep 2026, "Lanes A and B Revised 130926": "This is not a prototype. Please create it in production for real"; migration 091). **Lane A** is *only people I invite*, **lane B** is *anyone with the link*, and they are **eight screens, four steps after the branch** — the same screens, differing only *inside* `C2` and `C6`, so the lanes cannot drift. **Who can come is step 1** because it decides four steps or ten; its cards each carry a moss consequence line and a tinted notice counting what is left, both derived from `stepsFor` rather than written down, and a shared screen shows both totals ("2 of 4 · 2 of 10 public") from `publicSteps`. **There is no price step**: "Is anyone paying" is a drawer that opens in place over the chosen answer, carrying the mode boxes, the amount, the minimum and maximum, the refund rule when Epic holds the money, and — when the price depends on numbers — the paired range with `EACH PAYS` attached beneath each end. Epic holds the highest per-head price and refunds the difference once numbers are final; below the minimum nothing runs and everybody is refunded. Per person only: a household booking three places pays three times. **Every offer carries its own invitation link** from the first tap (`host_offers.link_token`): `/i/<token>` names one offer, and for an invite-only offer that token opens the page exactly as a personal invitation's does (`?l=`) — nothing else does. **My Epic contacts** (`host_contacts`) is everyone a household has invited, matched on a **mobile or an email and never on a name**, so two people who share a name and not a number stay two rows; a person the search finds in both the address book and Epic shows once, as the Epic row. Everyone invited is saved there, and the next event's search leads with the people invited most often. A new contact needs a name and **one of** a mobile or an email; with neither, both fields go red and the message says why. Adding people pops the count, says so in a lime bar and tints the new rows for about two and a half seconds. Coming back to `C2` once the event is live drops the progress bar, titles itself with the event, shows each person's answer, and finishes with Done. **Lane C — listed on Epic — was explicitly out of scope in that package** and must not be built from it.
...
**ID is asked once, at the gate, and only after two yeses** (O9) — not to be in the pool. **Nobody clears their own**: a person sends a photograph of their ID and a selfie, the check waits at `pending`, and the **back office** passes it or sends it back with a reason (`open_id_checks`, migration 096; Back office › Hosting › ID checks) — the same place a host's trust is set. Only a pass sets that side's timestamp, and only two passes open the chat, which is the only path to `chat` there is. "We check it and keep nothing but the result" is literal: both images are deleted the moment somebody decides, pass or fail. **An identity provider is the owner's to choose** — this is Epic's own eyes, not a verification service. **Home addresses are never shared and a first meet is never at one.** Family to family: daytime, somewhere public, both families present throughout, both adults ID-checked, ages as bands, and **no child's name or photograph and no matching or filtering by a child's sex**.

**What a side is told about the other's journey** is the destination, the town they are travelling from and the **month** — never a date and never an address. **A place comes from the map, never from typed text**: `townFor` reverse-geocodes the household's home or the trip's base (zoom 12 — shallower returns a council) and an entry with no coordinates carries no town, because a household's `home_label` is whatever they typed and is usually the house. `placeName` is the gate before anything crosses to the other side — one clean place name, no commas, no digits — and migrations 100 and 101 cleared every label written before that was true. Seven review passes went into an address parser before this; telling a house from a town by its words cannot be done, and the structured data was there all along. The card counts the people already asked (`liveMatchCounts`) and never lists them.
...
**A hello and an ID photograph are not public media.** Both live in `host_media` beside a host's photograph, and `GET /api/media/:id` is public by design — so they carry `is_private` and that handler refuses them. **Media is private unless somebody says otherwise** (migrations 097 and 098): the default was the other way round, which is how a hello and a passport ended up on the public reader, and the only thing that says otherwise is an upload whose **purpose is `listing`** — a host's photograph, a listing's video, the document guests download and a review's picture, all drawn on pages anybody may open. `POST /api/host/media` takes that purpose and defaults to private, and the client has no default at all — every call site names it, because the server failing safe while the client quietly said "listing" was the same hole one layer up (migrations 098 and 099); **evidence** (a qualification, a licence) is private, which is what the wizard already promises on screen. Evidence is never drawn, so its payload reports *whether* one was uploaded rather than an address that would not work. Recording a hello again deletes the one it replaces, rather than leaving an orphan row nobody points at. They are reachable only from their own guarded addresses: a hello from `/api/open/matches/<id>/hello/<mine|theirs>` behind `videoVisible`, an ID image from `/api/admin/open/checks/<id>/<doc|selfie>` behind the back office's door and **only while the check is `pending`** — a replacement sent after a fail is a draft until the person sends it. Both are `no-store`, and both screens fetch the bytes with the session and hold them as a blob they revoke, so neither becomes a URL in anybody's history.
...
**Seed sources are read once and are never a runtime dependency.** `vocabulary_sources` records what each gives us, its licence, the exact attribution wording at import time, and whether the data itself may be kept — which, because we take identifiers and write our own labels, is usually no. Only the CC BY and ODC-By sources oblige anything on screen, never Wikidata. Craft Courses, ClassBento, Meetup and the Heritage Crafts inventory were **read by hand once and rewritten in Epic's voice**: no scheduled crawler, no ingestion of anyone's list as live data, no refresh job pointed at anybody. Writing your own list informed by public pages is ordinary product work; the *compilation* is what carries weight, and that is not engaged by reading four pages once.

**The craft inventory is in, in full** (owner, 14 Sep 2026: "It's just a list of skills. There's nothing to protect here, and I asked for us to be able to just extract that list and utilise it… We don't need to send an email"). Migration 104 adds the remaining 221 names, taking Crafts and making to 257 tags — the long tail the whole layer exists for, since no marketplace taxonomy contains *withy pot making*. **The grading that source publishes is not taken and no column holds it**: whether a craft is endangered is a judgement about the craft rather than a fact about the skill, and Epic has no use for it (owner, same day).
...
**Completeness is recorded, never inferred.** `max(minutes)` is not a cap anybody built to. `cell_builds` holds what each origin was built to, so an interrupted rebuild is visible as two caps at once; and because a rebuild replaces markers as cells finish, the **run row** is what proves a build reached the end — a mode whose last run is anything but `done` is reported as needing one, however current its markers read, **including a mode whose first build died before it wrote a single row**. The report is scoped to one scheme and country, so a second country sharing the table cannot mask a new sector or invent a missing one.
...
**This confirms §13.16 and closes the question left open by the rating bench.** A provider's figure is banded into one of four words at the moment of the call and discarded there; the composite is ours, retained, and **recomputed rather than adjusted**, which is the answer to "how do we change our score three months later if we never kept the original rating" — nothing is ever changed. `ownedScore` is kept beside it: the same ranking with the licensed input removed, which is the proof the ordering survives a provider going dark.

**Where a provider's figure is and is not persisted, stated exactly** (audited 17 Sep 2026; an earlier draft of this section claimed it was nowhere, which is wrong and would have been unsafe to rely on). It never reaches the **owned layer** — not `place_records`, `place_facts`, `scout_places`, `attractions` or `place_ledger` — and the only `ratings` table in the schema holds the household's own. It **is** held in one place: `plan_sessions.state`, the jsonb blob of ideas a planning session is resumed from, which carries the provider's venue names and ratings and lives for **ten hours** (migration 026, owner 4 Sep 2026: "10 hours is also fine, so yes, please do that", and his own description of it on 17 Sep: "we could only retain Google's stars for 10 hours or whatever it is"). That is a bounded cache of rented content and never becomes an owned record; the ten hours is the whole of "come back to what you left", and shortening it shortens that.

**Two numbers, because they are two different things, and this paragraph states the mechanism rather than a guarantee.**

*Readable* is ten hours, and it is enforced in the queries rather than promised: `livePlanSession` will not return a row past `expires_at`, and `planSessionByRef` — which is deliberately not expiry-filtered, because "no run begins that" and "that run expired" are different answers — hands `expires_at` back so the route answers **410 with nothing derived from the state**. Both had to be got right: the run endpoint used to read an expired session's state and answer from it, which served the provider's content past the window (Codex, 17 Sep 2026). **Any new reader of `plan_sessions` must check the expiry itself**; that is the cost of keeping the two answers apart.

*Physically present* is longer, and depends on the API running. The sweep leaves an hour's grace, so a household returning a minute late is told the session expired rather than having a row vanish mid-request, and then deletes on the next pass. That pass was daily, which made the worst case about **thirty-five hours** depending on what time of day a session happened to be created; it is **hourly** now, so a running API clears a row within about twelve hours of its creation. It is a best-effort sweep on a process timer, not a durable job: if the API is stopped the rows stay until it comes back. That is the honest statement, and the reason it is tolerable rather than alarming is that **nothing can read them while the API is down** — the ten hours above is the guarantee that matters, and it is enforced in SQL rather than by a timer.

An interval is part of a retention policy whether or not anybody wrote it down as one, and a paragraph meant to govern a licensing decision should describe what the code enforces and where it stops, not make a promise the code cannot keep.
...
**The working has to add up to the number it claims to explain**, and at first it did not: two accolades worth 0.98 printed as 1.0 and the total came out a tenth high. `score()` now returns its parts unrounded beside the rounded ones, and a test asserts the sum over five shapes of place. The inputs must be the ones the score was actually made from — the sweep's own `website`, `chain_scale` and `sites` **in that order of preference**, not an owned record's website and a scale re-derived from the name, or the page explains a score nobody ever calculated. `rescore()` reads `scout_places.website`, so a swept place with a different website on its owned record must still be explained by the one that scored it.
