...
**Builds on:** the back-office design language set on 12 September 2026 — `Categories.tsx` is the reference screen, and §8 below is the law. Reuse the Epic pack v1: lime, ink, cream, Archivo only, square corners, 2px rules, no shadows, no red except allergens, overruns and the Plan screen's Stop.
...
## 5. The spine: one grammar, drilled all the way down
...
- **ready** — enough data to describe this place properly to a household (§7).
...
- **Coverage** — what's missing. Inherits today's six-fact grid, but see §7.
...
## 7. The ready bar — design this as a composer
...
## 9. Demand — and the rating workings

### 9a. Three numbers, not one conversion rate
...
## 9d. Household-made content: filtering and approval
...
7. The rating workings view (§7c), with Recalculate.
8. The ready-bar composer (§7), showing the effect before saving.
...
13. The moderation queue (§9d) — filtered, with one photograph and one review open.
...
may belong with the moderation work in §9d rather than with the atlas, since both are a
