...
export const TIPS = {
  ...
  censusFloor: ['This count is a floor', 'Google answers with its top sixty and stops. A slice that came back full was truncated, so at least one question here could not be answered whole even after the census split it down to the smallest tile it will go to. The real number is higher than this one \u2014 how much higher, nobody can say from here, and an Unresolved figure beside it does not cap it: that bucket is places the census did return, not the ones it never saw.'],
  ...
  censusSweeping: ['Not every tile has answered', 'This count was written while a tile planned for the ground had not answered for this drawer \u2014 a census still sweeping, or one that ended with a tile it could not finish. The number is a floor of a floor \u2014 true as a minimum, false as an answer \u2014 and it is drawn with the tiles beside it so it cannot be read, sorted or compared as a finished count. It sits below the finished rows whatever its size. When the last tile lands the row is written again and reads as the others do.'],
  censusUnresolved: ['Unresolved', 'Places whose census box straddles the edge of this area. The census is IDs Only, so a place has no point of its own: it is placed by the box it was found in, and a box wider than the line can be on either side of it. These are neither counted nor dropped. Read the two numbers together: the true figure is somewhere from the count up to the count plus this, either end included \u2014 every straddler may be in, or every one out \u2014 and nobody knows the split until the census asks that ground at a finer size. Neither the count alone nor the sum is known to be the answer; either could turn out to be. A district smaller than a box will show most of its places here until then. On this board\u2019s 30- and 60-minute views the figure is blank: a place straddling two districts that are both in the reach is inside it, and the per-district figures cannot be added without counting it twice \u2014 open one district to see it.'],
  censusSourced: ['How it was found', 'Type: Google guaranteed every place carries the type asked for. Words: words narrowing a real type. Text: a bare text query with no type to fence it \u2014 open a few before trusting that number. Mixed: more than one of these.'],
