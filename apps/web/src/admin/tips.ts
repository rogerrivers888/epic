...
export const TIPS = {
  ...
  beenThere: ['Been there', 'Review counts added up across every provider that has returned this place. A fact about how busy it is, not how good it is.'],
  ...
  fault: ['Fault', 'Which of the three numbers on the left is the worst, named plainly. No places — we hold none, so nothing could be shown. Wrong places — we showed some and nobody opened them, so the mapping is off. Thin places — they were opened and nobody went, so the records are too sparse to convince.'],
  ...
  howWeGotToHigh: ['How we got to high', '3 providers returned a rating for Windsor Castle, 12,480 reviews between them. Weighted by count the mean is 4.6 across all three, and 4.4 once the smallest provider is dropped for having under 30 reviews. 4.4 sits in the 4.2 to 4.5 band → high. Only the word is kept.'],
  howWeGotToVeryBusy: ['How we got to very busy', 'Those same 12,480 reviews, added across the 3 providers, put Windsor Castle in the busiest 5% of anything in the county → very busy. Busy is the next 15%, then steady, then quiet. The figure itself is not stored.'],
  ...
  notAsked: ['Not asked', 'We have never spent a call asking Google about this place, so we hold no id for it there.'],
  ...
  ourSubcategory: ['Our subcategory', 'One of our 59. Every one is listed whether or not anything landed in it, because an empty one is the finding.'],
  ...
  rating: ['Rating', 'A word, not a figure. At the moment of the call we read every provider\'s rating and review count, weight the ratings by count and pick a band: 4.6 and above is top, 4.2 to 4.5 high, 3.5 to 4.1 mixed, below that poor. Thorpe Park: 3 providers, 41,208 reviews, mean 4.7 → top. The mean and the counts are then thrown away and only the word is kept.'],
  ...
  subcategory: ['Subcategory', 'Which of our 59 that place sits in.'],
