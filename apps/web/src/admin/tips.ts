...
export const TIPS = {
  ...
  censusFloor: ['This count is a floor', 'Google answers with its top sixty and stops. A slice that came back full was truncated, so at least one question here could not be answered whole even after the census split it down to the smallest tile it will go to. The real number is higher than this one \u2014 how much higher, nobody can say from here. Where an Unresolved figure sits beside it, the real number is somewhere between the count and the count plus the unresolved; neither end is the answer.'],
  ...
  censusUnresolved: ['Unresolved', 'Places whose census box straddles the edge \u2014 of this area, or of the ring. The census is IDs Only, so a place has no point of its own: it is placed by the box it was found in, and a box wider than the line can be on either side of it. These are neither counted nor dropped. Read the two numbers together: the true figure sits somewhere between the count and the count plus this \u2014 the count alone is not the answer, and the sum is not the answer either, because the bucket splits, some in and some out, and nobody knows the split until the census asks that ground at a finer size. A district smaller than a box will show most of its places here until then.'],
