...
export const TIPS = {
  ...
  censusUnresolved: ['Unresolved', 'Places whose census box straddles the edge of this area. The census is IDs Only, so a place has no point of its own \u2014 it is placed by the box it was found in, and a box wider than the area can be on either side of the line. These are neither counted nor dropped. A district smaller than a box will show most of its places here until the census asks that ground at a finer size. On a ring the figure is not shown: a place straddling two districts that are both in the ring is inside it, and the per-district counts cannot tell which \u2014 open one district to see it.'],
