/**
 * Google Places (New) — Table A, the place types a place can be typed as.
 *
 * A hand-kept copy of the published list, grouped the way Google groups it,
 * read on `CHECKED_ON` from
 * https://developers.google.com/maps/documentation/places/web-service/place-types.
 * It exists so the Categories screen can show every word Google *could* say
 * about a place, not only the ones `google.js` happens to read; a type that is
 * here and in no map in `google.js` is a type Epic is currently ignoring, and
 * that is a fact worth being able to see. A type Google adds after this date
 * still arrives — the vocabulary counts whatever a real place carries
 * (repositories/taxonomyLabels.js) — it just has no group until somebody adds
 * it here.
 *
 * This is a vocabulary, not content: the list is Google's public
 * documentation, and nothing about any place is kept from it.
 */

export const CHECKED_ON = '2026-09-12';

export const GOOGLE_TYPE_GROUPS = {
  Automotive: ['car_dealer', 'car_rental', 'car_repair', 'car_wash', 'ebike_charging_station', 'electric_vehicle_charging_station', 'gas_station', 'parking', 'parking_garage', 'parking_lot', 'rest_stop', 'tire_shop', 'truck_dealer'],
  Business: ['business_center', 'corporate_office', 'coworking_space', 'farm', 'manufacturer', 'ranch', 'supplier', 'television_studio'],
  ...
  Education: ['academic_department', 'educational_institution', 'library', 'preschool', 'primary_school', 'research_institute', 'school', 'secondary_school', 'university'],
  ...
  Government: ['city_hall', 'courthouse', 'embassy', 'fire_station', 'government_office', 'local_government_office', 'neighborhood_police_station', 'police', 'post_office'],
  'Health and Wellness': ['chiropractor', 'dental_clinic', 'dentist', 'doctor', 'drugstore', 'general_hospital', 'hospital', 'massage', 'massage_spa', 'medical_center', 'medical_clinic', 'medical_lab', 'pharmacy', 'physiotherapist', 'sauna', 'skin_care_clinic', 'spa', 'tanning_studio', 'wellness_center', 'yoga_studio'],
...
/** Every type, with the group Google files it under. */
