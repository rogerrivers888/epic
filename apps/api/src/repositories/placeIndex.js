...
export async function checkBars({ repair = true, trigger = 'manual' } = {}) {
  ...
  const out = { ranAt, trigger, bare: [], inherited: [], stillBare: [], unjudged: [], rescored: 0, left: [], orphans: [], error: null };
  try {
    out.orphans = (await orphanedRules()).map((r) => ({ id: r.id, scope: r.scope, subject: r.subject, subcategory: r.subcategory }));
