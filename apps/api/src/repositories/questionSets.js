...
export async function globalFromCandidate(id, { label = null, kind = 'yesno', refreshDays = null, actor = null } = {}) {
  return withTransaction(async (client) => {
    ...
    const { rows: local } = await client.query(
      // Any set question on the label, on or off: an active one would ask the
      // fact twice, a switched-off one strands its answers or can be switched
      // back on beside the global (Codex, 26 Sep 2026, three rounds).
      "select set_key from questions where attribute_key = $1 and scope = 'set'", [key],
    );
