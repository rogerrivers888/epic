...
export async function list({ namespace = null, q = null, seenOnly = false, limit = 500, offset = 0 } = {}) {
  ...
  const { rows } = await query(
    `with all_labels as (
       select namespace, key, label, note, seen_count, active, seeded, decision, points_at, first_seen from taxonomy_labels
       union all
       select 'wikidata', qid, label, category, seen_count, admit, true, null, points_at, null::timestamptz from place_kinds
       union all
       ...
       select 'epic', key, label, 'a primary label — one thing a place is',
              ...
              active, seeded, null, null, created_at
         from shelf_subcategories
       union all
       select 'epic', key, label, 'a secondary label — something true about a place', 0, active, seeded, null, null, created_at
